...
namespace KalanBlazor.BL.Ventas
{
    public class CFDi
    {
        ...
        private byte[] ImprimirCFDi(string XMLString, string Sucursal, string Orden, string Observaciones, string Moneda)
        {
            ...
            var textoQr = $"https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?id={ExtraValor(XMLString, "TimbreFiscalDigital", "UUID")}&re={ExtraValor(XMLString, "Emisor", "Rfc")}&rr={ExtraValor(XMLString, "Receptor", "Rfc")}&tt={ExtraValor(XMLString, "Total")}&fe={sellosat}";
            string svg = QrCode.EncodeText(textoQr, QrCode.Ecc.Medium).ToSvgString(0);
        ...
        public string ImprimirAcuse(List<AcuseDatos> datos, string Documento, string carpetaPDF)
        {
            ...
            var PDFBytes = Reporte.Render("PDF");
            var ruta = Path.Combine(carpetaPDF, $"{Documento}_Acuse.pdf");
        ...
        public async Task<DTOs.Ventas.CFDi.CancelarResponse2> CancelarSAT(string Documento, string Motivo)
        {
            ...
            var carpeta = (new FileInfo(factura.RutaPDF)).Directory.FullName;
            var rutaAcuse = ImprimirAcuse(respuestaSAT.AcuseDatos, Documento, carpeta);
            ...
            var existingEntityCC = contexto.Set<th_cuentascobrar>().Find(factura.Id);
            if (existingEntityCC != null)
            {
                contexto.Entry(existingEntityCC).CurrentValues.SetValues(cuenta);
            }
            else
            {
                contexto.Attach(cuenta);
            }
      ...
      .FirstOrDefault();
        ...
        public string GenerarXMLComplemento(int Id_Ingreso)
        {
            ...
            var ingreso = contexto.th_ingresos
                ...
                .Include(x => x.cliente).ThenInclude(x => x.usocfdi)
                .Include(x => x.cliente).ThenInclude(x => x.regimenfiscal)
                .Include(x => x.th_cuentascobraraplicaciones)
                .ToList();
            var folio = contexto.th_facturacion_series.Where(x => x.Serie == "CP").FirstOrDefault();
            var folioCompleto = $"{folio.Serie.ToString()}-{folio.FolioSiguiente.ToString("D9")}";
            folio.FolioSiguiente++;
            contexto.th_facturacion_series.Update(folio);
            contexto.SaveChanges();
            var tComprobante = ingreso
                .Select(x =>
                new DTOs.Ventas.CFDi.Comprobante
                {
                    Version = "4.0",
                    Serie = SerieGet(folioCompleto).Replace("-", ""),
                    Folio = FolioGet(folioCompleto).Replace("-", ""),
                    Fecha = x.Fecha,
                    Certificado = "",
                    Descuento = 0,
                    Exportacion = "01",
                ...
                .ToList();
            var trelacion = new List<DTOs.Ventas.CFDi.Relacionado>();
            var tEmisor = new List<ComprobanteEmisor> { new ComprobanteEmisor { Nombre = "KALAN CORP MNA", RegimenFiscal = "601", Rfc = "KCM210429N64" } };
            var tReceptor = new List<ComprobanteReceptor> { new ComprobanteReceptor { Nombre = ingreso.First().cliente.RazonSocial, Rfc = ingreso.First().cliente.RFC, UsoCFDI = "CP01", DomicilioFiscalReceptor = ingreso.First().cliente.CodigoPostal, RegimenFiscalReceptor = ingreso.First().cliente.regimenfiscal.Id } };
            if (string.IsNullOrEmpty(ingreso.First().cliente.NumRegIdTrib) == false)
            {
                tReceptor.First().NumRegIdTrib = ingreso.First().cliente.NumRegIdTrib;
            }
            if (string.IsNullOrEmpty(ingreso.First().cliente.ResidenciaFiscal) == false)
            {
                tReceptor.First().ResidenciaFiscal = ingreso.First().cliente.ResidenciaFiscal;
            }
            var dtRelacionado = new DataTable();
            dtRelacionado.Columns.Add("IdDocumento", typeof(System.Guid));
            dtRelacionado.Columns.Add("Serie", typeof(string));
            dtRelacionado.Columns.Add("Folio", typeof(string));
            dtRelacionado.Columns.Add("MonedaDR", typeof(string));
            dtRelacionado.Columns.Add("EquivalenciaDR", typeof(decimal));
            dtRelacionado.Columns.Add("NumParcialidad", typeof(int));
            dtRelacionado.Columns.Add("ImpSaldoAnt", typeof(decimal));
            dtRelacionado.Columns.Add("ImpPagado", typeof(decimal));
            dtRelacionado.Columns.Add("ImpSaldoInsoluto", typeof(decimal));
            dtRelacionado.Columns.Add("ObjetoImpDR", typeof(string));
            dtRelacionado.Columns.Add("ImpuestoPagadoMF", typeof(decimal));
            dtRelacionado.Columns.Add("ImpuestoPagadoMO", typeof(decimal));
            dtRelacionado.Columns.Add("ImportePagadoMF", typeof(decimal));
            dtRelacionado.Columns.Add("ImportePagadoMO", typeof(decimal));
            dtRelacionado.Columns.Add("PagoImportePagadoMO", typeof(decimal));
            dtRelacionado.Columns.Add("PagoImportePagadoMF", typeof(decimal));
            dtRelacionado.Columns.Add("PagoBasePagadoMO", typeof(decimal));
            dtRelacionado.Columns.Add("PagoBasePagadoMF", typeof(decimal));
            dtRelacionado.Columns.Add("PagoIVAPagadoMF", typeof(decimal));
            dtRelacionado.Columns.Add("PagoIVAPagadoMO", typeof(decimal));
            dtRelacionado.Columns.Add("Factor", typeof(float));
            dtRelacionado.Columns.Add("Factor2", typeof(float));
            dtRelacionado.Columns.Add("ImpuestoPagadoMFExento", typeof(decimal));
            dtRelacionado.Columns.Add("ImpuestoPagadoMOExento", typeof(decimal));
            dtRelacionado.Columns.Add("ImportePagadoMFExento", typeof(decimal));
            dtRelacionado.Columns.Add("ImportePagadoMOExento", typeof(decimal));
            dtRelacionado.Columns.Add("ImpuestoPagadoMF16", typeof(decimal));
            dtRelacionado.Columns.Add("ImpuestoPagadoMO16", typeof(decimal));
            dtRelacionado.Columns.Add("ImportePagadoMF16", typeof(decimal));
            dtRelacionado.Columns.Add("ImportePagadoMO16", typeof(decimal));
            dtRelacionado.Columns.Add("TC_Rec", typeof(decimal));
            dtRelacionado.Columns.Add("TC_Fac", typeof(decimal));
            dtRelacionado.Columns.Add("SOPNUMBE", typeof(string));
            return "";
        }
        ...
        public DTOs.SAT.Timbrado.Comprobante DataSetToComprobante(DataSet datos)
        {
            ...
            foreach (DataRow rw2 in datos.Tables[0].Rows)
            {
                ...
                comprobante.Total = decimal.Round(Convert.ToDecimal(rw2["Total"]), 2, MidpointRounding.ToEven);
